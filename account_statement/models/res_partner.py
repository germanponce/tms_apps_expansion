...
class AccountInvoice(models.Model):
	...
	def _get_result(self):
		for aml in self:
			aml.result = 0.0
			aml.result = aml.amount_total - aml.credit_amount 

	def _get_credit(self):
		for aml in self:
			aml.credit_amount = 0.0
			aml.credit_amount = aml.amount_total - aml.amount_residual_signed
...
class Res_Partner(models.Model):
	...
	supplier_invoice_ids = fields.One2many('account.invoice', 'partner_id', 'Customer move lines', domain=[('type', 'in', ['in_invoice','in_refund']),('state', 'in', ['posted'])]) 
	balance_invoice_ids = fields.One2many('account.invoice', 'partner_id', 'Customer move lines', domain=[('type', 'in', ['out_invoice','out_refund']),('state', 'in', ['posted'])]) 
